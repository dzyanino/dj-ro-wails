...
export const useEdgeStore = defineStore("edges", () => {
  ...
  function addEdge(edge: EdgeWithId) {
    if (!edge || !edge.id || !edge.source || !edge.target) {
      console.error(
        "Invalid edge object. It must have an 'id', 'source', and 'target' properties."
      );
      return;
    }
    ...
    if (edges.value[edge.id]) {
      console.warn(
        `Edge with id ${edge.id} already exists. Updating the edge.`
      );
    }

    const duplicateEdge = Object.values(edges.value).some(
      (e) => e.source === edge.source && e.target === edge.target
    );
    if (duplicateEdge) {
      console.warn(
        `Edge with source ${edge.source} and target ${edge.target} already exists.`
      );
    }
  ...
  function updateEdge(edge: EdgeWithId) {
    ...
    if (edges.value[edge.id]) {
      ...
      console.warn(
        `Edge with id ${edge.id} does not exist. Adding it instead.`
      );
      addEdge(edge);
    }
  }
