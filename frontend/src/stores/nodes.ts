...
export const useNodeStore = defineStore("nodes", () => {
  ...
  function addNode(node: NodeWithId) {
    if (!node || !node.id || !node.name) {
      console.error(
        "Invalid node object. It must have an 'id' and 'name' properties."
      );
      return;
    }
    ...
    const duplicateName = Object.values(nodes.value).some(
      (n) => n.name === node.name
    );
    if (duplicateName) {
  ...
  function updateNode(node: NodeWithId) {
    ...
    if (nodes.value[node.id]) {
      ...
      console.warn(
        `Node with id ${node.id} does not exist. Adding it instead.`
      );
      addNode(node);
    }
  }
